...
export const SideBarMenuItem = ({ item }: { item: SideNavItem }) => {
  ...
  const inactiveLink = classNames(
    "flex items-center min-h-[40px] h-full text-white py-2 px-4 hover:text-sidebar-muted-foreground  hover:bg-rchsLight rounded-md transition duration-200",
    { ["justify-center"]: toggleCollapse }
  );

  const activeLink = classNames(
    "rounded-md text-white light:text-black light:bg-rchs bg-rchsDark text-sidebar-muted-foreground"
  );

  const navMenuDropdownItem =
    "text-black py-2 px-4 hover:text-white transition duration-200 rounded-md";
